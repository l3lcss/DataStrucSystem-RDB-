  ...
  <div>
    <div class="columns is-centered">
      <div class="column is-4" style="margin: auto;text-align:center">
        <img src="https://media.giphy.com/media/3o6fJeVJSTSXqzwaSk/giphy.gif" width="60%">
      </div>
          ...
          <table class="table">
            ...
            <tbody>
              <tr v-for="(schedule, key) in userDetails" :key="key">
                ...
                <td v-else-if="(getUserLogin.schedule.time !== schedule.time && (getUserLogin.schedule.TA !== '' && getUserLogin.schedule.TA)) || (getUserLogin.schedule.TA !== '44' && getUserLogin.schedule.TA !== '')">
                  <b-field>
                    <b-radio-button
                      disabled
                ...
                <td v-else-if="!getUserLogin.schedule.TA || getUserLogin.schedule.TA === '44'">
                  <b-field>
                    <b-radio-button
                      v-model="schedule.nativeValue"
                      :native-value="true"
  ...
  methods: {
    ...
    async initData () {
      ...
      this.userDetails = taData.schedules.map((time) => {
        let schedules = {}
        if (userData.schedule.TA === '44' && userData.schedule.time === time.time && userData['.key'] === time.ID) {
          schedules = {
            ...time,
            nativeValue: true
          }
